...
public class Backend {
    ...
    public void refreshPrices() {
        sendGetRequest("/evaluate", "",
                (String response) -> data = JsonParser.parseString(response).getAsJsonObject(),
    ...
    public void sendGetRequest(final String path, final String params, Consumer<String> onResponse, Consumer<Exception> onError) {
        new Thread(() -> {
            try {
                ...
                AttributeMod.mc.execute(() -> onError.accept(e));
            }
        }).start();
    }

    public void sendPostRequest(final String path, final String params, final String body, Consumer<String> onResponse, Consumer<Exception> onError) {
        new Thread(() -> {
            try {
                URL url = buildUrl(path, params);
                HttpsURLConnection con = (HttpsURLConnection) url.openConnection();
    ...
    private void handleResponse(Consumer<String> onResponse, HttpsURLConnection con) throws IOException {
        ...
        con.disconnect();
        AttributeMod.mc.execute(() -> onResponse.accept(content.toString()));
    }

    private URL buildUrl(String path, String params) throws URISyntaxException, MalformedURLException {
        String key = AttributeMod.config.modkey.trim();
        String uuid = Helper.getPlayerUUID();

        return new URI(URL + path + "?key=" + key + "&uuid=" + uuid + "&version=" + AttributeMod.VERSION + params).toURL();
    }
